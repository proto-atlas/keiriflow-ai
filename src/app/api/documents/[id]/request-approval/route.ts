...
type RequestApprovalRouteContext = {
  params: Promise<{
    id: string;
  }>;
};

export async function POST(request: Request, context: RequestApprovalRouteContext) {
  ...
  try {
    const document = await repository.requestApproval(id, {
    ...
    return Response.json({ error: "approval_request_failed" }, { status: 500 });
  }
}
